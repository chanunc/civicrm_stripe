      ...
      var isWebform = false;
        ...
        cj(function() {
          cj(document).ready(function() {
            ...
            console.log("BillingBlock.tpl called!!!");
            /*
            ...
            // For CiviCRM 4.5 and Webform
            if(cj('#crm-main-content-wrapper>form').length){
              cj('#crm-main-content-wrapper>form').addClass('stripe-payment-form');
            }else if(cj('form[id^="webform-client-form-"]').length){
              cj('form[id^="webform-client-form-"]').addClass('stripe-payment-form');
              // Hide billing Middle Name field as will be used for passing stripe-token
              cj('.billing_middle_name-section').hide();
              isWebform = true;
            }
          ...
          function stripeResponseHandler(status, response) {
            if (response.error) {
              ...
              //Start: For Webform
              if(isWebform){
                // Add 'op' with value: 'Submit'
                var input = cj("<input>").attr("type", "hidden").attr("name", "op").val("Submit");
                cj("form.stripe-payment-form").append(cj(input));

                //DEBUG
                //console.log(token);

                // Put stripe token to billing_street_address-5
                cj('#billing_middle_name').val(token);
              }
              //End: For Webform
